...
'@
    ...
    $desktopDeadline = [DateTime]::UtcNow.AddSeconds(2)
    while ([DateTime]::UtcNow -lt $desktopDeadline -and
        [StartupDeferralAcceptanceNative]::GetForegroundWindow() -ne $desktopWindow) {
        Start-Sleep -Milliseconds 20
    }
    if ([StartupDeferralAcceptanceNative]::GetForegroundWindow() -ne $desktopWindow) {
        throw "未能把前台焦点交给 Windows 桌面。desktop=$desktopWindow, foreground=$([StartupDeferralAcceptanceNative]::GetForegroundWindow())"
    }
