...
播放器每 500 毫秒检查一次前台窗口。只要其他普通程序位于前台，无论它是普通大小、最大化还是全屏，音乐都会自动暂停；关闭、最小化该程序或回到桌面后，只恢复本次被自动暂停的歌曲。原本由用户手动暂停的歌曲不会被自动播放。播放器自身窗口、任务栏、开始菜单和 Windows 桌面右键菜单不会触发暂停，窗口模式和桌面模式采用相同规则。
...
从托盘退出时，程序会先从 WorkerW 桌面层解除播放器，再要求 Explorer 重绘桌面，因此不会改动或清空 Windows 原有壁纸设置。
