...
页面加载完成后会自动播放当前默认歌曲，不需要先点播放键。若系统临时拒绝自动播放，程序保持可操作，仍可手动点击播放。

播放器每 500 毫秒检查一次前台窗口。其他程序进入标准最大化或真正全屏状态时，音乐会自动暂停；窗口还原、退出全屏、关闭或用户回到桌面后，只恢复本次被自动暂停的歌曲。进入最大化前已经手动暂停的歌曲不会被自动播放。多显示器上的最大化和无边框全屏都受支持，窗口模式和桌面模式采用相同规则。
